...
import "./interfaces/IUniBaseV3Vault.sol";
import "./uni/interfaces/INonfungiblePositionManager.sol";
import "./uni/interfaces/ISwapRouter.sol";
import "./uni/interfaces/IUniswapV3PoolState.sol";
import "./uni/interfaces/IWKLAY.sol";
import "./libraries/Abs.sol";
import "./libraries/PathEncode.sol";
import "./libraries/Array.sol";
import "./libraries/TransferHelper.sol";

contract UniBaseV3Vault is IUniBaseV3Vault, ReentrancyGuard {
    ...
    IWKLAY private _iwklay;

    bytes[] token0path;
    bytes[] token1path;

    address[] public airdropList;
    mapping(address => bytes) airdropPath;
    ...
    constructor(
        ...
        factoryOwner = IUniBaseV3Factory(_factory).factoryOwner();
        npmId = _npmId;
        pool = _pool;
        WKLAY = _WKLAY;

        _iwklay = IWKLAY(_WKLAY);
    ...
    function pushAirdropToken(
        address _token,
        address[] memory pathList,
        uint24[] memory feeList
    ) external onlyFactoryOwner {
        if (_token == WKLAY) {
            airdropList.push(_token);
        } else {
            IERC20(_token).approve(address(_irouter), type(uint256).max);

            airdropList.push(_token);
            airdropPath[_token] = new bytes(0);
            airdropPath[_token] = PathEncode.pathEncode(pathList, feeList);
        }
    }
    ...
    function withdraw(
        ...
        if (token0 == WKLAY) {
            ...
            _inpm.unwrapWETH9(amount0Result, address(this));
            _inpm.sweepToken(token1, amount1Result, address(this));
            ...
            _inpm.unwrapWETH9(amount1Result, address(this));
            _inpm.sweepToken(token0, amount0Result, address(this));
        } else {
            (amount0Result, amount1Result) = _collect(
                address(this),
                uint128(amount0),
    ...
    function _harvest() internal notFinish {
        ...
        if (token0 == WKLAY) {
            ...
            _inpm.unwrapWETH9(amount0, address(this));
            _inpm.sweepToken(token1, amount1, address(this));

            uint256 amountOut = _swapFromERC20(amount1, token1path[0]);

            wklayTotal += amountOut;
            amountTotal += (amountOut + amount0);
        } else if (token1 == WKLAY) {
            (amount0, amount1) = _collect(
                address(0),
                type(uint128).max,
            ...
            _inpm.unwrapWETH9(amount1, address(this));
            _inpm.sweepToken(token0, amount0, address(this));

            uint256 amountOut = _swapFromERC20(amount0, token0path[0]);

            wklayTotal += amountOut;
            amountTotal += (amountOut + amount1);
        } else {
            (amount0, amount1) = _collect(
                address(this),
                type(uint128).max,
            ...
            wklayTotal += (amount0Out + amount1Out);
            amountTotal += (amount0Out + amount1Out);
        }
        ...
        for (uint i = 0; i < airdropList.length; i++) {
            ...
            if (token0 == WKLAY || token1 == WKLAY) {
                balanceToken = IERC20(airdropList[i]).balanceOf(address(_inpm));

                if (balanceToken == 0) continue;

                _inpm.sweepToken(airdropList[i], balanceToken, address(this));
            } else {
                balanceToken = IERC20(airdropList[i]).balanceOf(address(this));

                if (balanceToken == 0) continue;
            }

            uint reward;
            if (airdropList[i] == WKLAY) {
                reward = balanceToken;
            } else {
                reward = _swapFromERC20(
                    balanceToken,
                    airdropPath[airdropList[i]]
                );
            }
        ...
        if (wklayTotal != 0) _iwklay.withdraw(wklayTotal);
    ...
    {
        (, int24 currentTick, , , , , ) = IUniswapV3PoolState(pool).slot0();
    ...
    function _increase(
        ...
        returns (uint128 liquidity, uint256 amount0, uint256 amount1)
    {
        (liquidity, amount0, amount1) = _inpm.increaseLiquidity{value: value}(
            params
        );

        if (token0 == WKLAY || token1 == WKLAY) _inpm.refundETH();
    }

    function _decrease(
        INonfungiblePositionManager.DecreaseLiquidityParams memory params
    ) internal notFinish returns (uint256 amount0, uint256 amount1) {
        (amount0, amount1) = _inpm.decreaseLiquidity(params);
